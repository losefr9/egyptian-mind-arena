  ...
  ELSE
    ...
    IF p_game_id = (SELECT id FROM games WHERE name = 'XO' LIMIT 1) THEN
      INSERT INTO xo_matches (
        ...
        v_new_session_id,
        v_opponent_queue_entry.user_id,  -- البدء بالفور الأول (من كان في الانتظار)
...
BEGIN
  ...
  -- إرجاع الرصيد (في حالة كان مخصوماً)
  -- في النظام الجديد لن نخصم الرصيد إلا عند المطابقة
...
-- دالة للحصول على المعلومات العامة للخصم
