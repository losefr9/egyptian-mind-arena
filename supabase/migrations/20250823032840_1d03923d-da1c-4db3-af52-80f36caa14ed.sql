...
-- إدراج لعبة XO إذا لم تكن موجودة (بدون conflict)
INSERT INTO games (name, description, is_active) 
SELECT 'XO Game', 'لعبة إكس أو مع أسئلة رياضية - اجب على السؤال الرياضي لوضع علامتك!', true
WHERE NOT EXISTS (SELECT 1 FROM games WHERE name = 'XO Game');
...
-- إدراج أسئلة رياضية عشوائية فقط إذا لم تكن موجودة
INSERT INTO math_questions (question, answer, difficulty_level) 
SELECT * FROM (VALUES
  ('15 + 28', 43, 1),
  ('23 + 42 + 12', 77, 2),
  ('67 - 29', 38, 1),
  ...
  ('72 ÷ 9', 8, 1),
  ('56 + 29 - 14', 71, 2)
) v(question, answer, difficulty_level)
WHERE NOT EXISTS (SELECT 1 FROM math_questions WHERE question = v.question);
